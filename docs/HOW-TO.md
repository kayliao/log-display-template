...
在頁面的 `filter_bar` 加一行讓它可以收起來：

```php
View::component('filter_bar', [
    'id'          => 'logFilter',
    'target'      => 'logTable',
    'collapsible' => true,     // 標題列變成開關，按一下整排條件收合
    'collapsed'   => false,    // 一進頁面是展開的；改成 true 就一進來先收著
    'fields'      => $fields,
]);
```

收起來的時候「查詢」「清除」會跟著藏起來，這是故意的 ——
留一顆查詢鈕但看不到條件，按下去不會知道自己查了什麼。

狀態不會被記住，重新整理就回到 `collapsed` 設定的樣子。
現場好幾個人輪流用同一台電腦，記住上一個人的狀態只會讓下一個人以為壞掉了。
...
1. **`key` 一定要是唯一值。** 勾選記的是識別碼不是那顆 checkbox，
   重複值會變成勾一筆、另一筆也跟著亮。
...
| 匯入說「這個檔案不是文字檔，也不是可以匯入的表格」，並附上開頭位元組 | 檔案根本不是 CSV 也不是 xlsx，只是副檔名叫那樣。xlsx 的開頭一定是 `50 4B 03 04`（`PK`，它是 ZIP）；不是的話就不是 xlsx | 看訊息裡的開頭位元組去對格式，或在伺服器上跑 `file 檔名`。常見來源是系統匯出的自訂格式、或有密碼保護的檔案 |
