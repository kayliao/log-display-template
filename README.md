...
`Csv::read()` 處理掉現場最常見的三個坑：**編碼**、**UTF-8 BOM**、
**逗號還是 Tab 分隔**。所以現場不用先轉檔。
...
> ⚠ UTF-16 一定要在 Big5 之前判斷。少了這一段，UTF-16 的檔案會掉進 Big5
> 那條路被硬轉，變成一整片亂碼**而且不會拋錯**——使用者只會看到
> 「目前讀到的欄位是：j盬鋑_?」這種訊息，看不出問題出在編碼。
> 沒有 BOM 的 UTF-16 靠空位元組的位置認：UTF-16 存 ASCII 字元（CSV 一定有的
> 逗號、數字、換行）會固定配一個 `\0`，而 UTF-8 與 Big5 的文字檔不會出現 `\0`。
